...
export default function Chat(){
  const { t } = useI18n();
  const [messages, setMessages] = useState([{role:'assistant', content:t["chat.welcomeMessage"]}]);
  const [text, setText] = useState('');

  const send = () => {
    if(!text.trim()) return;
    setMessages(m => [...m, {role:'user', content:text}, {role:'assistant', content:t["chat.demoMessage"]}]);
    setText('');
  };
